...
class Parser
{
    ...
    public function parse($unparsedString, $defaultTld = 'com')
    {
        try {
            ...
            if ($matchedDomain == '' && strlen($matchedDomainIdn) <= 63 && $matchedTld == '') {
                ...
                throw AbstractException::factory('UnparsableString', 'Unparsable domain name.');
            }
    ...
    private function load()
    {
        ...
        if (! $existFile || $this->reload === true) {
            ...
            if ($file === false) {
                throw AbstractException::factory('OpenFile', 'Could not open cache file.');
            }

            if (fwrite($file, serialize($this->tldList)) === false) {
                throw AbstractException::factory('WriteFile', 'Could not open cache file for writing.');
            }
    ...
    private function catchTlds($existFile)
    {
        ...
        if ($content === false) {
            if (! $existFile) {
                throw AbstractException::factory('Connect', 'Could not catch file from server.');
            }
        ...
        if (preg_match('/\/\/ ===BEGIN ICANN DOMAINS===(.*)(?=\/\/ ===END ICANN DOMAINS===)/s', $content, $matches) !== 1) {
            throw AbstractException::factory('UnparsableString', 'Could not fetch ICANN Domains of Mozilla TLD File.');
        }
